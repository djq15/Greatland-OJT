...
if (isset($_POST['save'])) {
	...
	$query = "INSERT INTO ieti_db.math101 (student_id, first_name, last_name, grade) VALUES ('$studentid', '$firstname', '$lastname', '$grade')"; //hit
	...
	header('location: gradesubmissionmath101.php'); //hit
}
	...
	mysqli_query($db, "UPDATE ieti_db.math101 SET prelim='$prelim', midterm='$midterm', final='$final' WHERE student_id = $studentid"); //hit
	...
	mysqli_query($db, "DELETE FROM ieti_db.math101 WHERE student_id = $studentid"); //hit
	...
	header('location: gradesubmissionmath101.php');	//hit
		...
		$results = mysqli_query($db, "SELECT student_id, first_name, last_name, prelim, midterm, final FROM ieti_db.math101, ieti_db.login where username=first_name and tag='2'");
		//hit
	...
	$rec = mysqli_query($db, "SELECT * FROM ieti_db.math101 WHERE student_id=$studentid");	 //hit
...
<table>
	...
	<tbody>
		...
		<tr>
			...
			<td>
				<a class = "edit_btn" href="gradesubmissionmath101.php?edit=<?php echo $row['student_id']; ?>">Edit</a> <!-- hit -->
			</td>
