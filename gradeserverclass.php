...
if (isset($_POST['save'])) {
	...
	$results = mysqli_query($db, "SELECT username, password, tag, evaluation_submit, clearance_approve, evaluation, first_name FROM ieti_db.login, ieti_db.subject1 where username = first_name and username='$currentUser'");
	//$results = mysqli_query($db, "SELECT * FROM login where username='$currentUser'");
		...
		if ($row=mysqli_fetch_array($results)) {
			...
			$results = mysqli_query($db, "SELECT student_id, first_name, last_name, prelim, midterm, final FROM ieti_db.subject1, ieti_db.login where username = first_name and username='$currentUser'");
