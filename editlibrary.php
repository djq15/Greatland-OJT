...
if (isset($_POST['save'])) {
	...
	mysqli_query($db, "UPDATE greatland_db.login SET username='$username', password='$password', tag='$tag', clearance_approve='$approval', email='$email', existing_folder='$folder', verified='$verified' WHERE username = '$username'"); //hit
	...
	*/
	...
	$results = mysqli_query($db, "SELECT * from greatland_db.library");
...
<table style="white-space: nowrap;">
	...
	<tbody>
		...
		<tr>
			...
			<td>
				<a class = "edit_btn" href="javascript:deleteme('<?php echo $row['book_code']; ?>')" name="delete">Delete</a> <!-- hit -->
			</td>
...
{
	if (confirm('Do you want to delete book code "'+delid+'" ?')){
			window.location.href='deletelibrary.php?del_id=' +delid+'';
			return true;
	}
}
</script>
...
if (isset($_POST["yes"])) {
	...
	mysqli_query($db, "DELETE FROM greatland_db.login WHERE student_id='$studentid'"); //hit
	$_SESSION['msg'] = "Details deleted";
